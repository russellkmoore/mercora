...
│
├── themes/                       # ← Theme files (data only, no logic); barrel is generated
│   ├── volt-dark.css             # Default preset (predates the direction doc, relocated verbatim)
│   ├── [luxe|midnight|clinical|retro|atelier|market].css # Six more presets
│   └── index.generated.css       # ← Generated barrel (do not edit; scripts/build-themes.mjs)
│
...
│   ├── themes/                   # ← Theme resolution + 23-token contract access
...
│   │   └── manifest.generated.ts # Generated theme manifest (do not edit; run build-themes.mjs)
│   ├── layout/                   # ← Layout switch resolution
│   │   ├── settings.ts           # ← getLayoutSettings() — the three enumerated layout switches
│   │   └── variants.ts           # Layout enums (CategoryLayout, HomeHero, ProductGallery)
...
│   │   ├── ThemePresetGrid.tsx   # ← Theme selection grid (writes appearance.theme)
│   │   ├── LayoutSwitches.tsx    # ← Layout switch radiogroups (writes the three layout keys)
│   │   └── [admin UI]/
│   ├── layout/                   # ← Layout variant components (components/layout/)
│   │   ├── category/             # CategoryGrid3, CategoryGrid2, CategoryList + layout map
│   │   ├── home/                 # HomeHeroFullBleed, HomeHeroSplit, HomeHeroMinimal + layout map
│   │   └── product/              # ProductGalleryLeft, ProductGalleryTop
...
│   ├── build-themes.mjs          # ← Validates themes/*.css, generates manifest + barrel (also --check)
│   ├── scan-hardcoded-colors.mjs # ← Token scanner (npm run scan:tokens); local gate, not wired into CI
│   ├── screenshot-routes.mjs     # ← Multi-viewport, multi-theme screenshot harness
│   └── [other scripts]/
│
├── tests/                        # Test suite
...
**`themes/`**
- Purpose: Theme files — data only, one `[data-theme="<name>"]` CSS rule per file, no logic
- Contains: Seven preset `.css` files plus the generated import barrel
- Key files: `volt-dark.css` (default), `index.generated.css` (← generated by `scripts/build-themes.mjs`, do not edit)

**`lib/themes/`**
- Purpose: Theme resolution and the 23-token contract's server-side access
- Contains: The active-theme resolver, the generated manifest, and the camelCase token accessor for the four non-cascade consumers
- Key files: `active-theme.ts` (`getActiveTheme()`), `manifest.generated.ts` (← generated), `tokens.ts` (`getThemeTokens()`)

**`lib/layout/`**
- Purpose: The three enumerated layout switches (category grid, home hero, product gallery) and their request-time resolution
- Contains: The layout settings resolver and the enum definitions each layout-variant map keys against
- Key files: `settings.ts` (`getLayoutSettings()`), `variants.ts` (the three enums + defaults)
...
**Theming & Layout:**
- `lib/themes/active-theme.ts`: Active theme resolver (`getActiveTheme()`) — D1 → env default → manifest default
- `lib/layout/settings.ts`: Layout switches resolver (`getLayoutSettings()`)
- `lib/layout/variants.ts`: The three layout enums and their defaults
- `scripts/build-themes.mjs`: Theme validator + manifest/barrel generator (also `--check`)
- `scripts/scan-hardcoded-colors.mjs`: Token scanner (`npm run scan:tokens`)
- `scripts/screenshot-routes.mjs`: Multi-viewport, multi-theme screenshot harness
- See `docs/theming.md` for the full theming system
...
**New Theme:**
1. **Copy an existing theme file** → `themes/<name>.css` (`cp themes/volt-dark.css themes/my-theme.css`)
2. **Set the `@theme` metadata line** → `label` (required), `industry`, `synopsis`
3. **Set the selector** → `[data-theme="<name>"]` must match the filename stem
4. **Set all 23 tokens** → the 17 hex colours, 4 radius tokens, 2 font tokens
5. **Run the theme build and the token scanner** → `node scripts/build-themes.mjs && npm run scan:tokens`

See `docs/theming.md`'s "Duplicate a theme in five steps" for the full version, including what
the validator rejects and why.
