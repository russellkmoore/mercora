...
**Applied:** 2026-09-09T23:55:00Z
**Source:** milestone integration check (two content follow-ups, no code changes)

Two content corrections. No file under `lib/`, `app/` or `components/` was
touched. `12-CONTEXT.md` D-09 was deliberately left alone, as instructed.

**Commits:**

| Commit | What |
|--------|------|
| `3995ec5` | Article: limit the Account gift-card listing claim to a signed-in purchase |
| _(this section)_ | Terms §6 timing — production-only D1 change, recorded here |

### Follow-up 1: Terms §6 gift-card send timing (D1 `pages`, slug `terms-of-service`)

The live section said gift cards are "delivered to the recipient by email as
soon as payment completes" and omitted the scheduled delivery date — the same
claim CR-01 and CR-03 corrected on the article and the product copy. This was
the last surface still asserting an unconditional immediate send.

Production-only: the `pages` row is edited through Admin → Pages and has no
committed source in the repo, so there is nothing to commit for it. The record
is this section.

**How it was applied.** One statement, generated by a scratch script from the
row as it had just been read back, so no prose was re-typed. The script refuses
to write unless the target phrase occurs **exactly once** — a second occurrence
would mean the section had drifted since the read, and a blind replace could
silently rewrite the wrong sentence. It also round-trips the edit (putting the
old phrase back must reproduce the original byte for byte) before emitting any
SQL, which is what proves nothing outside the replaced phrase moved.

New sentence:

> Gift cards are delivered to the recipient by email as soon as payment
> completes, **or at the start of the delivery date (UTC) chosen when the card
> is bought**. They never expire, cannot be redeemed for cash, and cannot be
> transferred or resold. Stored value can be used across multiple orders. If you
> have a question about a gift card, please contact us. Gift card terms version
> 2026-09-09.
...
| | length | md5 |
|---|---|---|
| **before** | 1468 | `4c0a0ef39a1bc0c50f557eac877eaadb` |
| **after** | 1543 | `01bff21d7e4f1fe8513089ca190190ff` |

Delta is exactly +75 characters, which is the phrase delta and nothing else.
The read-back `after` md5 equals the value computed locally before applying, so
production matches the intended content byte for byte.

```
"changes": 2, "rows_read": 1, "rows_written": 1, "changed_db": true
```

`version` 3 → 4, `updated_at` 1788985950 → 1788997907, exactly as 12-03 moved
them. Every other page row is untouched:

```
about=v1  faq=v2  fleet-services=v1  privacy-policy=v1  terms-of-service=v4
```

**Live page:**

```
$ curl -s https://voltique.russellkmoore.me/terms-of-service | grep -o 'delivery date (UTC)' | wc -l
3
```

Counted with `/usr/bin/grep` against the saved response and cross-checked in
Node — a piped `curl | grep` in this shell is rewritten by the `rtk` hook and
returns 0 regardless of content, which is a measurement artefact, not a cache
miss. The page sends `cache-control: private, no-cache, no-store`, so there is
no cached copy to wait out.

**Restore SQL** (1847 bytes, generated from the same before/after pair so it
cannot drift from the change it undoes). `version` moves forward rather than
back: it is an ever-increasing edit counter and rewinding it would misrepresent
the row's history — the same reasoning 12-03 recorded.

```sql
-- Restore for the Phase 12 Terms section 6 follow-up.
...
<p>For questions about these terms, please contact us.</p>', version = version + 1,
  updated_at = CAST(strftime('%s','now') AS INTEGER)
WHERE slug = 'terms-of-service' AND status = 'published';
```

Restore verified, not merely written: the content literal was extracted back out
of the generated SQL, un-doubled, and hashed — 1468 bytes,
`4c0a0ef39a1bc0c50f557eac877eaadb`, byte-identical to the capture.
`RESTORE_ROUNDTRIP_OK`.

### Follow-up 2: The Account listing claim applies only to a signed-in purchase

**Files modified:** `data/r2/knowledge_md/gift-cards.md`,
`tests/unit/data/knowledge-gift-cards.test.ts`
**Commit:** `3995ec5`

WR-04 narrowed this claim from "shoppers can see their gift cards" to "a shopper
who bought a gift card can see it". Still too wide.
`listCustomerGiftCardPresentations` filters on
`account.purchaser_customer_id`, which comes from the order's `customer_id` and
is **null for a guest purchase** — so a guest-bought card is never listed there,
not at the time and not later if the buyer signs up. `/api/gift-cards` also 401s
without a Clerk session.

That left two ways to hold a card and find an empty page: receive one, or buy
one as a guest. The article named only the first.

> A gift card bought while signed in appears under Account, then Gift cards,
> with its remaining balance. A card bought as a guest is not listed there,
> because it was never linked to an account. A card someone received does not
> appear there either — for safety the code is only ever sent to the recipient's
> email address.

A new assertion pins the signed-in qualifier and the guest exclusion, so the
claim cannot quietly widen again.
...
|------|--------|
| `wrangler r2 object put ... knowledge_md/gift-cards.md --remote` | Upload complete |
| ETag vs `md5 -q` | both `ef43f88ebb004c755c344ae4daf694be` — match |
| Re-index | 9 walked, **1 upserted**, 0 errors; `textLength=2438` (= the committed file exactly); `mutationId=346d1c4e-50f4-4815-b7e4-cd18c94092b5` |
| `vectorize info` | dimensions 768, **vectorCount 48**, `processedUpToMutation` = `346d1c4e-...` |

**On what `get-vectors` can and cannot show here.** The edited sentence sits at
character **1755** of the article, and the indexer stores only the first 1000
characters as `metadata.text` — the admin route's own truncation rule, left
alone. So `get-vectors` cannot display the new sentence, and the stored preview
is byte-identical to the previous upsert's. That is expected, not a failed
write. Two things prove the re-index really took the new text:

1. The stored preview is byte-identical to the **committed file's** first 1000
   characters (`md5 a897de18a1e35070b60e05e9766c71f3` on both sides), so the
   vector was built from the file as committed.
2. The **embedding changed**, and it can only have changed because the text did.
   Comparing the 768 values against the vector captured before this upsert:

   ```
   components changed: 763 of 768
   max component delta: 4.738e-3
   cosine similarity before/after: 0.999020
   ```

**Retrieval check** (read-only query against the live index, no writes):

```
Q: can I see my gift card balance in my account if I checked out as a guest?
   0.7051  knowledge-gift-cards
   0.6493  prod_33
Q: when is a gift card with a delivery date actually sent?
   0.7237  knowledge-gift-cards
   0.6957  knowledge-shipping
```

Both questions retrieve the article first, with the product vector also in the
top three — and both now carry consistent wording, which was the point of CR-03.

### Gates

| Gate | Result |
|------|--------|
| `npx vitest run tests/unit/data/knowledge-gift-cards.test.ts` | 13 passed |
...
_Follow-ups applied: 2026-09-09_
_Fixer: Claude (gsd-code-fixer)_
