...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 3-01-01 | 01 | 1 | ADR-01, ADR-02 | T-03-02 | Doc claim is grep-checked against the code before it is written | grep | `test "$(grep -c 'remains outside' docs/checkout-trust-boundary.md)" = "0"` | ✅ | ✅ green |
| 3-01-01 | 01 | 1 | ADR-02 | T-03-01 | Lock record survives into git | shell | `test -n "$(git ls-files -- gsd-ingest-manifest.yaml)"` | ✅ | ✅ green |
| 3-01-02 | 01 | 1 | ADR-02 | T-03-01 | Marker is dated and machine-checkable | grep | `test "$(grep -lE '^\*\*Status:\*\* Accepted \(20[0-9]{2}-[0-9]{2}-[0-9]{2}\)$' docs/checkout-trust-boundary.md docs/webhooks-refunds-inventory.md docs/database-migrations.md docs/subscriptions.md \| wc -l \| tr -d ' ')" = "4"` | ✅ | ✅ green |
| 3-01-02 | 01 | 1 | ADR-02 | T-03-01 | Manifest still parses and no non-ADR entry was locked | node | `node -e "…yaml parse: 26 entries, 4 ADR locked, 0 non-ADR locked…"` (full command in 03-01-PLAN.md) | ✅ | ✅ green |
| 3-02-01 | 02 | 1 | RUN-01 | T-03-04 | No unguarded production migration instruction remains | grep | `test "$(grep -n 'wrangler d1 migrations apply' docs/DEPLOYMENT_SETUP.md \| grep -vc -- '--local')" = "0"` | ✅ | ✅ green |
| 3-02-01 | 02 | 1 | RUN-02 | T-03-05 | Every documented event has a handler | grep | `test "$(grep -c '^   - \`' docs/DEPLOYMENT_SETUP.md)" = "16"` | ✅ | ✅ green |
| 3-02-02 | 02 | 1 | RUN-01 | T-03-04 | Same gate applied to the second runbook | grep | `test "$(grep -n 'wrangler d1 migrations apply' docs/CLAUDE.md \| grep -vc -- '--local')" = "0"` | ✅ | ✅ green |
| 3-02-03 | 02 | 1 | RUN-02 | T-03-05 | The two operator lists cannot drift | shell | `test "$(grep -c '^   - \`' docs/STRIPE_INTEGRATION.md)" = "16" && test "$(grep '^   - \`' docs/DEPLOYMENT_SETUP.md)" = "$(grep '^   - \`' docs/STRIPE_INTEGRATION.md)"` | ✅ | ✅ green |
| 3-03-01 | 03 | 1 | RUN-02 | T-03-07, T-03-08 | Removal is behaviour-neutral; signature path untouched | unit | `npx vitest run tests/unit/app/api/stripe-webhook-signature.test.ts tests/unit/app/api/stripe-webhook-payment-failed.test.ts tests/unit/app/api/stripe-webhook-subscription-route.test.ts tests/unit/app/api/stripe-webhook-subscriptions.test.ts tests/unit/app/api/stripe-webhook-refunds.test.ts tests/unit/app/api/stripe-webhook-retry.test.ts` | ✅ | ✅ green |
| 3-03-02 | 03 | 1 | RUN-02 | T-03-09 | Unhandled event still claimed, ignored, 200, no finalizer | unit | `npx vitest run tests/unit/app/api/stripe-webhook-unhandled-events.test.ts` | ✅ (created by 3-03-02) | ✅ green |
| 3-03-02 | 03 | 1 | RUN-02 | — | No orphaned import or type after the deletion | build | `npm run lint && npm run typecheck && npm test` | ✅ | ✅ green |
...
- [x] `nyquist_compliant: true` set in frontmatter — set by the validate-phase audit on 2026-09-02
...
State A audit by the orchestrator after phase completion. Every gate in the Per-Task Verification Map was re-run against the completed tree and passed: the seven scripted CLI gates for ADR-01, ADR-02, RUN-01, and RUN-02 (no `remains outside`; manifest tracked; four dated `Status: Accepted` markers; no unguarded production `wrangler d1 migrations apply` in either runbook; 16 identical event bullets in both runbooks), and the seven-file webhook vitest run including the new `stripe-webhook-unhandled-events.test.ts` (7 files, 65 tests, all green). Post-merge gates during execution: `npm run build` clean, `npm test` 242 files / 1868 tests green, regression run of 11 prior-phase test files green, `npm run lint` and `npm run typecheck` clean. The one manual-only item (ingest classifier re-run) was executed on throwaway branch `tmp/ingest-check` and passed; record in `03-UAT.md`. No Wave 0 was needed and no auditor was spawned.
