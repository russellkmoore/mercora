...
### Research resolutions (Claude, after 14-RESEARCH.md)
- **D-19:** `issueAccount` derives `issuance_business_key` from the new card's own id, so D-06/D-07's literal keys are not writable. Idempotency comes from a **deterministic new-card id** instead: reissue uses `giftCardReissueId(oldId)` (a stable derivation, e.g. a UUIDv5-style hash of `gift-card-reissue/{oldId}`), admin-create uses the event id minted once per request; the UNIQUE id plus the D-01 partial index keep reissue once-only. The `adjustment` entry on the old card carries business key `gift-card-reissue-out/{oldId}` through a new `writeAdjustment` repository method modelled on `restoreRedemption`'s idempotent INSERT. Both writes go in one D1 `batch()`; an integration test proves a second reissue attempt fails and leaves no partial state.
- **D-20:** `app/api/admin/settings/route.ts` currently refuses the whole `gift_cards` category. Narrow the refusal to the `HONOR_GUARD_SETTING_KEY` key (and any key equal to it after normalisation) so `gift_cards.code_reveal_enabled` can be saved there; the route keeps referencing `HONOR_GUARD_SETTING_KEY`, so the Phase 13 write contract stays green, and `admin-settings-honor-guard.test.ts` gains a case that the reveal key is accepted while the guard key is still 400.
- **D-21:** Resend needs an exported `resendGiftCardDelivery({ deliveryId, to, idempotencyKey, environment, now })` in `lib/services/gift-card-fulfillment.ts` that reuses the private `deliveryMessage`/decrypt path; the route passes the full worker env (`getCloudflareContext().env`), never a `{ DB }`-only object.
- **D-22:** Confirm dialogs and toasts follow `components/admin/reviews/ReviewModerationDashboard.tsx` and `app/admin/orders/ShipmentModal.tsx` (Dialog/AlertDialog + sonner), not the orders detail page's `window.confirm`. "Purchaser" in the list is a label resolved from the customer row's JSON person (name, else email), or "admin: {display name}" for admin-created cards; the raw customer id is not shown.
