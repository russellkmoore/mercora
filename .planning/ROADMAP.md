...
- [ ] **Phase 1: Security and Admin-Auth Truth** - Scrub published secrets, rotate the admin token, assert `NODE_ENV` at startup, and make every doc describe admin auth as it actually works
- [ ] **Phase 2: Observability and Regression Guards** - Give tax fallback and web vitals a real signal, type the slug routes with 404 tests, widen allocation tests, resolve the webhook TODO, and record a Lighthouse baseline
...
Plans:
- [ ] 01-01-PLAN.md — Deployment guard tracer: taxonomy event, `lib/auth/deployment-guard.ts`, wired into `checkAdminPermissions` with tests (SEC-03)
...
- [ ] 01-03-PLAN.md — Credential scrub and doc truth across the three admin-auth documents (SEC-01, SEC-04)
- [ ] 01-04-PLAN.md — Rotate `ADMIN_VECTORIZE_TOKEN` and prove the published value is dead on the live site (SEC-02)
...
|-------|----------------|--------|-----------|
| 1. Security and Admin-Auth Truth | 0/4 | Planned | - |
