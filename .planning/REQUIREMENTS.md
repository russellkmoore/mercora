...
- [ ] **THEME-05**: The four remaining presets from the theme direction doc (`clinical`, `retro`, `atelier`, `market`) ship as validated 23-token theme files with header metadata, their display fonts load via `next/font`, each is screenshot-captured across the route grid, and the light ones pass scrim/shadow QA; admin Appearance lists all seven themes
