...
- **D-05:** When a `stripeCustomerId` is available and a PaymentIntent is actually created (the non-zero-cash branch; the zero-cash gift-card path at line ~242 is untouched), `createPaymentIntent` is called with `customer: stripeCustomerId`. `setup_future_usage` is **not** hardcoded — see D-06/Research for whether it is set by us or by the Payment Element's Customer Session save-card checkbox; this is a research-resolved mechanic, not guessed.
- **D-06 (research-resolved, placeholder until 17-RESEARCH.md):** The exact mechanism for (a) letting the shopper opt into saving the card via a checkbox in the Payment Element, and (b) showing that shopper's previously-saved cards in the Payment Element, must be verified against current Stripe documentation (Customer Session + `payment_method.save_payment_method` vs. a plain `setup_future_usage` on the PaymentIntent) before planning. Do not guess the API shape — this phase requires research with library docs (context7 or Stripe's own docs), not training-data recall, given the API surface here is genuinely new to this codebase (`grep` confirms zero existing use of `customer_session`/`ephemeral`/`setup_future_usage`).
