...
- **D-06:** Evidence is gathered read-only from production D1 and the tail worker: the order row (`orders`), the `gift_card_accounts` row for that order line (issued amount 2500 USD, status active), the `gift_card_deliveries` row reaching `status = 'sent'` within two five-minute cron cycles, and no `cron.recovery_failed`. **Structural finding (research):** `GET /api/gift-cards` lists cards by `purchaser_customer_id`, which a guest purchase writes as NULL and which a signed-in purchase sets to the purchaser — never the recipient. So the "appears under Account → Gift Cards for the recipient's account" clause of SHOP-07 is not satisfiable by any purchase for a recipient other than the purchaser, and not by a guest purchase at all. **Unattended decision:** the proof covers issuance and the delivery email (the two things the backend promises) and records the account-listing clause as a documented product gap for Russell to decide on (recipient-email lookup would be a later change under `lib/gift-cards/`, out of this milestone's scope). If the test payment fails, the executor stops with the evidence rather than retrying blindly.
...
### UI design contract
- **D-11:** The plan-gate's frontend detector matched the word "page" in the success criteria; no template or component changes ship in this phase (content, index, one CMS row, one purchase), so planning runs with `--skip-ui`, the same call as Phase 9. — **Unattended decision**.
...
- **Product gap for Russell:** Account → Gift Cards shows cards by purchaser, not recipient; SHOP-07's wording implies the recipient. Deciding whether the recipient (matched by verified email) should see received cards is a `lib/gift-cards/` change for a later milestone.
- Production `EMAIL_PROVIDER` value is a Dashboard Build variable not visible from the repo; read the live delivery's provider field during the proof.
