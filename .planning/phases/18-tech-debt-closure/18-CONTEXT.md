...
### Research resolutions (Claude, after pattern-mapping)
- **D-15:** Nothing in the codebase actually gates/blocks checkout on `giftCardLineUnavailable` — it is advisory-only; the component's own comment says the server refuses the line regardless of what renders client-side. DEBT-03's `giftCardNoteInvalid` therefore needs no new blocking mechanism either — it gets the same advisory warning treatment, and checkout-pricing.ts's existing server-side gift-card validation is the real backstop (an invalid/unparseable customization was already going to be rejected there). This resolves D-03's open "Claude's discretion on the exact block mechanism" — there is none to add.
- **D-16:** DEBT-07's migration-collision check must fire only when a *newly added* migration file's number collides with an *existing* filename's number — not a blanket "any duplicate anywhere" scan, which would immediately fail CI on the already-applied, intentionally-tolerated `0023` pair. The check compares the diff's added files against the full existing file list. `scripts/lib/migration-safety.mjs` holds the testable pure logic (`inspectMigration`, `summarize`); the new check is a pure function there, tested via `tests/unit/scripts/migration-safety.test.ts`; `scripts/check-migration-safety.mjs` is the thin CLI wrapper.
- **D-17:** `app/api/payment-intent/route.ts:393` already unconditionally persists `billing_address: shippingAddress` regardless of digital-only status — confirmed, no change needed there (D-06's premise holds without modification).
- **D-18:** `lib/utils/email.ts` has two hardcoded "Shipping Address" headings (~241, ~436) and a third plain-text address block (~513-518) — all three need the same conditional label ("Billing address" only when the order was digital-only and `shipping_address` was null) as the account order-detail page.
