    ...
    <>
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-surface-elevated">
        {sections.map((section) => {
          ...
          return (
            <div key={section.id} id={section.id} className="scroll-mt-24 border-b border-border last:border-0">
              <h2>
                <button type="button" onClick={() => setOpenId(open ? null : section.id)} aria-expanded={open} aria-controls={`${section.id}-answer`} className="flex w-full items-center gap-3 px-5 py-4 text-left text-lg text-foreground hover:bg-surface-elevated">
                  <span className="flex-1">{section.heading}</span>
                  <ChevronDown aria-hidden className={`h-5 w-5 text-primary transition-transform ${open ? "rotate-180" : ""}`} />
                </button>
              </h2>
