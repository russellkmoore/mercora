...
export async function POST(req: NextRequest) {
  const { address, items }: { address: Address; items: CartItem[] } = await req.json();
  ...
  if(address.country !== "US") {
    return NextResponse.json({ error: "Shipping options only available for US addresses" }, { status: 400 });
  }
