...
export function generateOrderConfirmationText(orderData: OrderData): string {
  ...
  return [
    ...
    orderData.shippingAddress
      ? `${orderData.addressLabel === 'billing' ? 'Billing address' : 'Ship to'}: ${orderData.shippingAddress.street}, ${orderData.shippingAddress.city}, ${orderData.shippingAddress.state} ${orderData.shippingAddress.zipCode}, ${orderData.shippingAddress.country}`
...
export function generateOrderConfirmationHTML(orderData: OrderData, tokens: ThemeTokens): string {
  ...
  const itemsHTML = orderData.items.map(item => {
    ...
    return `
    <tr style="border-bottom: 1px solid ${tokens.borderInverse};">
      <td style="padding: 12px 0; vertical-align: top; width: 60px;">
        ${absoluteImageUrl ? `<img src="${escapeHtmlText(absoluteImageUrl)}" alt="${safeName}" style="width: 50px; height: 50px; border-radius: 4px; object-fit: cover; display: block;">` : `<div style="width: 50px; height: 50px; background-color: ${tokens.surfaceInverseElevated}; border-radius: 4px; display: flex; align-items: center; justify-content: center; color: ${tokens.mutedOnInverse}; font-size: 12px; text-align: center;">No Image</div>`}
      </td>
      <td style="padding: 12px 0 12px 16px; vertical-align: top;">
        <div style="color: ${tokens.onInverse}; font-size: 16px; font-weight: bold; margin: 0 0 4px;">${safeName}</div>
        <div style="color: ${tokens.mutedOnInverse}; font-size: 14px; margin: 0;">Quantity: ${escapeHtmlText(String(item.quantity))} × ${Money.fromStored(item.price).format()}</div>
      </td>
      <td style="padding: 12px 0; text-align: right; vertical-align: top;">
        <div style="color: ${tokens.onInverse}; font-size: 16px; font-weight: bold; margin: 0;">${Money.fromStored(item.price).times(item.quantity).format()}</div>
      </td>
    </tr>
  `;
  }).join('');
  ...
  return `
    ...
    <body style="margin: 0; padding: 0; background-color: ${tokens.surfaceInverse}; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Ubuntu, sans-serif;">
      <div style="background-color: ${tokens.surfaceInverseElevated}; margin: 0 auto; padding: 20px 0 48px; margin-bottom: 64px; max-width: 600px;">
        
        <!-- Header -->
        <div style="text-align: center; padding: 32px 0; border-bottom: 1px solid ${tokens.borderInverse};">
          <h1 style="color: ${tokens.primary}; font-size: 32px; font-weight: bold; margin: 0; padding: 0;">${safeStoreName}</h1>
          <p style="color: ${tokens.mutedOnInverse}; font-size: 14px; margin: 8px 0 0;">${safeTagline}</p>
        </div>

        <!-- Order Confirmation -->
        <div style="padding: 24px 32px;">
          <h2 style="color: ${tokens.onInverse}; font-size: 24px; font-weight: bold; margin: 0 0 16px;">Order Confirmed!</h2>
          <p style="color: ${tokens.mutedOnInverse}; font-size: 16px; line-height: 24px; margin: 0 0 16px;">Hi ${escapeHtmlText(orderData.customerName)},</p>
          <p style="color: ${tokens.mutedOnInverse}; font-size: 16px; line-height: 24px; margin: 0 0 16px;">Thank you for your order! It is being prepared and will be shipped soon.</p>
          
          <div style="background-color: ${tokens.surfaceInverseElevated}; border-radius: 8px; padding: 16px; margin: 16px 0;">
            <p style="color: ${tokens.onInverse}; font-size: 18px; font-weight: bold; margin: 0 0 8px;">Order #${escapeHtmlText(orderData.orderNumber)}</p>
            ${orderData.estimatedDelivery ? `<p style="color: ${tokens.mutedOnInverse}; font-size: 14px; margin: 0;">Estimated delivery: ${escapeHtmlText(orderData.estimatedDelivery)}</p>` : ''}
          </div>
        </div>

        <!-- Order Items -->
        <div style="padding: 24px 32px;">
          <h3 style="color: ${tokens.onInverse}; font-size: 18px; font-weight: bold; margin: 0 0 12px;">Your Items</h3>
        ...
        <!-- Order Summary -->
        <div style="background-color: ${tokens.surfaceInverseElevated}; border-radius: 8px; padding: 16px; margin: 24px 32px;">
          <table style="width: 100%;">
            <tr style="padding: 4px 0;">
              <td style="color: ${tokens.mutedOnInverse}; font-size: 14px;">Subtotal:</td>
              <td style="text-align: right; color: ${tokens.onInverse}; font-size: 14px;">${Money.fromStored(orderData.subtotal).format()}</td>
            </tr>
            <tr style="padding: 4px 0;">
              <td style="color: ${tokens.mutedOnInverse}; font-size: 14px;">Shipping:</td>
              <td style="text-align: right; color: ${tokens.onInverse}; font-size: 14px;">${Money.fromStored(orderData.shipping).format()}</td>
            </tr>
            ${orderData.discount && !Money.fromStored(orderData.discount).isZero() ? `
            <tr style="padding: 4px 0;">
              <td style="color: ${tokens.mutedOnInverse}; font-size: 14px;">Discount:</td>
              <td style="text-align: right; color: ${tokens.onInverse}; font-size: 14px;">${Money.fromStored(orderData.discount).negate().format()}</td>
            </tr>` : ''}
            <tr style="padding: 4px 0;">
              <td style="color: ${tokens.mutedOnInverse}; font-size: 14px;">Tax:</td>
              <td style="text-align: right; color: ${tokens.onInverse}; font-size: 14px;">${Money.fromStored(orderData.tax).format()}</td>
            </tr>
            ...
            <tr style="border-top: 2px solid ${tokens.borderInverse}; padding: 12px 0 0; margin: 12px 0 0;">
              <td style="color: ${tokens.onInverse}; font-size: 16px; font-weight: bold; padding-top: 12px;">Total:</td>
              <td style="text-align: right; color: ${tokens.primary}; font-size: 18px; font-weight: bold; padding-top: 12px;">${Money.fromStored(orderData.total).format()}</td>
            </tr>
          </table>
        </div>

        ${orderData.shippingAddress ? `
          <!-- ${orderData.addressLabel === 'billing' ? 'Billing' : 'Shipping'} Address -->
          <div style="padding: 24px 32px;">
            <h3 style="color: ${tokens.onInverse}; font-size: 18px; font-weight: bold; margin: 0 0 12px;">${orderData.addressLabel === 'billing' ? 'Billing Address' : 'Shipping Address'}</h3>
            <p style="color: ${tokens.mutedOnInverse}; font-size: 14px; line-height: 20px; margin: 0;">
        ...
        ` : `
          <div style="padding: 24px 32px;">
            <p style="color: ${tokens.mutedOnInverse}; font-size: 14px; line-height: 20px; margin: 0;">This order contains no shippable items.</p>
          </div>
        `}
  ...
  switch (orderData.status) {
    ...
    case 'shipped':
      ...
      statusContent = `
        ...
        ${orderData.carrier ? `
          <div style="background-color: ${tokens.surfaceInverseElevated}; border-radius: 8px; padding: 16px; margin: 16px 0;">
            <h3 style="color: ${tokens.onInverse}; font-size: 16px; font-weight: bold; margin: 0 0 8px;">Shipping Details</h3>
            <p style="color: ${tokens.mutedOnInverse}; font-size: 14px; margin: 0 0 4px;"><strong>Carrier:</strong> ${escapeHtmlText(orderData.carrier)}</p>
            ${orderData.trackingNumber ? `<p style="color: ${tokens.mutedOnInverse}; font-size: 14px; margin: 0 0 4px;"><strong>Tracking Number:</strong> ${escapeHtmlText(orderData.trackingNumber)}</p>` : ''}
            ${safeTrackingUrl ? `
              <a href="${escapeHtmlText(safeTrackingUrl)}" style="display: inline-block; background-color: ${tokens.primary}; color: ${tokens.onPrimary}; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold; margin-top: 12px;">
                Track Your Package
              </a>
            ` : ''}
          </div>
    ...
    case 'cancelled':
      ...
      statusContent = `
        ...
        ${orderData.cancellationReason ? `
          <div style="background-color: ${tokens.surfaceInverseElevated}; border-left: 4px solid ${tokens.danger}; padding: 12px 16px; margin: 16px 0;">
            <p style="color: ${tokens.danger}; font-size: 14px; margin: 0;"><strong>Reason:</strong> ${escapeHtmlText(orderData.cancellationReason)}</p>
          </div>
        ` : ''}
        <p style="color: ${tokens.mutedOnInverse}; font-size: 14px; line-height: 20px; margin: 0 0 16px;">If you have any questions about this cancellation or need assistance with a new order, please contact our support team.</p>
      `;
      break;
    ...
    case 'refunded':
      ...
      statusContent = `
        ...
        <div style="background-color: ${tokens.surfaceInverseElevated}; border-left: 4px solid ${tokens.primary}; padding: 12px 16px; margin: 16px 0;">
          <p style="color: ${tokens.primary}; font-size: 14px; margin: 0 0 4px;"><strong>Refund Processing:</strong></p>
          <p style="color: ${tokens.primary}; font-size: 14px; margin: 0;">Please allow 5-10 business days for the refund to appear on your statement.</p>
        </div>
        <p style="color: ${tokens.mutedOnInverse}; font-size: 14px; line-height: 20px; margin: 0 0 16px;">If you have any questions about this refund, please contact our support team.</p>
      `;
      break;

    default:
      statusMessage = "Order status updated";
      statusContent = `<p style="color: ${tokens.mutedOnInverse}; font-size: 16px; line-height: 24px; margin: 0 0 16px;">Your order status has been updated.</p>`;
  }
  ...
  const itemsHTML = orderData.items.slice(0, 3).map(item => {
    ...
    <tr style="border-bottom: 1px solid ${tokens.borderInverse};">
      <td style="padding: 8px 0; vertical-align: top; width: 50px;">
        ${absoluteImageUrl ? `<img src="${escapeHtmlText(absoluteImageUrl)}" alt="${safeItemName}" style="width: 40px; height: 40px; border-radius: 4px; object-fit: cover; display: block;">` : `<div style="width: 40px; height: 40px; background-color: ${tokens.surfaceInverseElevated}; border-radius: 4px; display: flex; align-items: center; justify-content: center; color: ${tokens.mutedOnInverse}; font-size: 10px; text-align: center;">No Image</div>`}
      </td>
      <td style="padding: 8px 0 8px 12px; vertical-align: top;">
        <div style="color: ${tokens.onInverse}; font-size: 14px; font-weight: bold; margin: 0 0 2px;">${safeItemName}</div>
        <div style="color: ${tokens.mutedOnInverse}; font-size: 12px; margin: 0;">Qty: ${escapeHtmlText(String(item.quantity))}</div>
      </td>
    </tr>
  `;
  }).join('');
  ...
  return `
    ...
    <body style="margin: 0; padding: 0; background-color: ${tokens.surfaceInverse}; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Ubuntu, sans-serif;">
      <div style="background-color: ${tokens.surfaceInverseElevated}; margin: 0 auto; padding: 20px 0 48px; margin-bottom: 64px; max-width: 600px;">
        ...
        <!-- Header -->
        <div style="text-align: center; padding: 32px 0; border-bottom: 1px solid ${tokens.borderInverse};">
          <h1 style="color: ${tokens.primary}; font-size: 32px; font-weight: bold; margin: 0; padding: 0;">${escapeHtmlText(store.identity.name)}</h1>
          <p style="color: ${tokens.mutedOnInverse}; font-size: 14px; margin: 8px 0 0;">${escapeHtmlText(store.identity.tagline)}</p>
        </div>
        ...
        <div style="padding: 24px 32px;">
          ...
          <div style="background-color: ${tokens.surfaceInverseElevated}; border-radius: 8px; padding: 16px; margin: 16px 0;">
            <p style="color: ${tokens.onInverse}; font-size: 16px; font-weight: bold; margin: 0 0 8px;">Order #${escapeHtmlText(orderData.orderNumber)}</p>
            <p style="color: ${tokens.mutedOnInverse}; font-size: 14px; margin: 0;">Status: <span style="color: ${statusColor}; font-weight: bold;">${escapeHtmlText(orderData.status.charAt(0).toUpperCase() + orderData.status.slice(1))}</span></p>
          </div>
          
          ${orderData.notes ? `
            <div style="background-color: ${tokens.surfaceInverseElevated}; border-radius: 8px; padding: 12px; margin: 16px 0;">
              <p style="color: ${tokens.mutedOnInverse}; font-size: 14px; margin: 0;"><strong>Note:</strong> ${escapeHtmlText(orderData.notes)}</p>
            </div>
          ` : ''}
        </div>
        ...
        <div style="padding: 24px 32px;">
          ...
          ${hasMoreItems ? `
            <p style="color: ${tokens.mutedOnInverse}; font-size: 12px; margin: 8px 0 0; text-align: center;">
...
export async function sendNewOrderMerchantNotification(
  ...
  const isBillingAddress = orderData.addressLabel === 'billing';
  const itemHeading = orderData.shippingAddress && !isBillingAddress ? 'Items to ship' : 'Order items';
  const addressHeading = isBillingAddress ? 'Billing address' : 'Ship to';
  const text = [
    ...
    `Tax: ${Money.fromStored(orderData.tax).format()}`,
    `Total: ${Money.fromStored(orderData.total).format()}`,
    orderData.shippingAddress ? addressHeading : null,
